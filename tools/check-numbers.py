...
# ---------- 画面に書いてある数と突き合わせる ----------
# (ファイル, 探す形, あるべき数, 何の数か)
# 画面の「札ごとの字数」は、**also（こちらでも出す札）を足した数**です。
# N2で絞ると、介護の札が本体で also:N2 の47字も出るので 320+47=367字。
# 一度ここを取りちがえて、正しかった注記（N2 367字）を 320字に直してしまいました。
# 正は kanji.json の level_totals（split-data.py が数えたもの）。
KLV = L('kanji.json')['level_totals']
KTOT = KTOTAL                      # 字の実体の数（重なりを 1つと数える）
KCOURSE = KLV['N5'] + KLV['N4'] + KLV['N3']
VTOT = sum(VC.values())
...
CHECKS = [
    ...
    # 入口メニューの説明文と、漢字画面の注記。ここは **見ていなかったので**
    # 「漢字1,066字と言葉6,272語」のまま 古くなっていた（語彙は6,309語だった）。
    ('index.html', r'漢字([\d,]+)字と言葉([\d,]+)語',
                                       (KTOT, VTOT), '入口メニューの説明文'),
    ('kanji.html', r'上記データセットの分類（累計([\d,]+)字）',
                                       KCOURSE, '漢字画面 累計'),
    ('kanji.html', r'N5 ([\d,]+)字・N4 ([\d,]+)字・N3 ([\d,]+)字・N2 ([\d,]+)字・介護 ([\d,]+)字'
                   r'・記録 ([\d,]+)字',
                                       (KLV['N5'], KLV['N4'], KLV['N3'], KLV['N2'],
                                        KLV['介護'], KLV['記録']), '漢字画面 レベルの内訳'),
    # 仕様書の1枚目の表。ここも見ていなかったので「4,558語」のまま古くなっていた。
    ('SPEC.md', r'\| 収録漢字 \| \*\*([\d,]+)字\*\*.*?の \*\*([\d,]+)字\*\*',
                                       (KTOTAL, KCOURSE), '仕様書 収録漢字'),
    ('SPEC.md', r'\| 収録語彙 \| \*\*([\d,]+)語\*\*', VTOT, '仕様書 収録語彙'),
    ('SPEC.md', r'\| 用例 \| のべ\*\*([\d,]+)語\*\*',
                                       sum(len(x.get('examples', [])) for x in kan),
                                       '仕様書 用例'),
    ('kanji.html', r'重なる([\d,]+)字は、どちらで絞っても出ます。字の実体は([\d,]+)字',
                                       (sum(1 for x in kan if x.get('also')), KTOTAL),
                                       '漢字画面 重なりの数'),
