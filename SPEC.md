...
| 収録語彙 | **3,801語**（N5 709・N4 662・N3 2,078・介護 227・現場 125）＋施設独自の追加分（`vocab-extra.csv`。現在1語で、アプリの表示は3,802語）。漢字語・かな語・カタカナ語を網羅 |
...
| ボタン | 出力 | 中身 |
|---|---|---|
| 📄 一覧をExcelで書き出す | `学習記録_YYYYMMDD.csv`（BOM付きUTF-8） | 漢字746行＋熟語3,801行。列は 種類／レベル／言葉・漢字／読み／意味／状態（覚えた・要復習・未学習）。Excelで直接開ける |
...
| 項目 | 仕様 |
|---|---|
| 例文データ | `sentences.json`（3,701語＝収録語彙3,801語の97%）。**介護197語・現場125語・N5 671語・N4 632語・N3 2,075語**をカバー。残る100語は「おはようございます」などの声かけフレーズ29語と、「～円」「～階」「～ばかり」「～観」などの接尾語・接尾表現71語で、それ自体が文・語尾のため対象外。値が文字列のときは見出し語がそのままの形で文中に出現、配列のときは `[例文, 文中の形, その読み]`（活用形に対応） |
...
| `vocab.json` | 約540 KB | 語彙3,801語（JLPT漢字語・かな語・カタカナ語＋介護＋現場） | 最新 |
