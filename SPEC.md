...
| 項目 | 仕様 |
|---|---|
| 出題形式 | JLPT語彙セクションに準拠した4択：**問題1 漢字読み**（語→読み）／**問題2 表記**（読み→漢字）／**問題3 文脈規定**（意味→語）／**問題4 漢字1字の読み**／総合（4形式を巡回） |
| 出題範囲 | レベル（すべて/N5/N4/N3/介護）× 問題数（10/20/30問） |
| 問題生成 | 収録データからその場で自動生成（追加データ不要）。誤答は**同レベル・語長が近い・読みが異なる語**から選定し、重複問題を排除 |
| 解答 | 選択直後に正誤を表示（正解＝緑、誤答＝赤）＋語の読み・意味を提示 |
| 採点 | 進捗バー・正解数／不正解数・最終得点（％）とコメント |
| 復習連携 | **間違えた語・漢字を自動で「要復習」に登録**（既存の学習記録と共通）。結果画面から「間違えた問題だけ」再挑戦可 |
| 負荷 | すべて端末内で生成。サーバー負荷・追加データともになし |
